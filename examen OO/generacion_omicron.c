...
void printInstancia(FILE * fpasm, int numero_atributos_instancia) {
    ...
    for(i = 0; i < numero_atributos_instancia; i++) {
        /*obtengo el offset del atributo*/
...
void copiaInstancia(FILE * fpasm, char * nombre, int numero_atributos_instancia) {
    ...
    if(fpasm == NULL)
        return;
    /*reservo memoria para la instancia*/
    instance_of(fpasm, nombre, numero_atributos_instancia);
    fprintf(fpasm, "\tpop dword eax\n");

    /*saco la direccion donde dejar el puntero reservado*/
    fprintf(fpasm, "\tpop dword ecx\n");
    /*asigno a la instancia el puntero reservado*/
    fprintf(fpasm, "\tmov [ecx], eax\n");

    /*saco la instancia a copiar de la pila*/
    fprintf(fpasm, "\tpop dword edx\n");
    fprintf(fpasm, "\tmov edx, [edx]\n");

    /*asigno a cada atributo de instancia el de la instancia copiada*/
    for(i = 0; i < numero_atributos_instancia; i++) {
        /*obtiene el valor del siguiente atributo a copiar*/
        fprintf(fpasm, "\tadd edx, 4\n");
        fprintf(fpasm, "\tmov ecx, [edx]\n");

        /*obtiene la direccion donde dejar el atributo a copiar*/
        fprintf(fpasm, "\tadd eax, 4\n");
